...
One of the most important considerations to make when programmatically accessing databases is how to secure your database credentials. **NEVER HARDCODE CREDENTIALS**. Each programming language provides means to externalize such information and bring it into the environment. For `Python`, the `config.ini` file is where you can store your database connection credentials to then be used by the script. \> Note: When using online repositories like `GitHub` or `Bitbucket`, be sure to configure your `.gitignore` files appropriately so that your externalized credentials files (e.g., `config.ini`, `.env`, etc.) are not uploaded with your code. See [here](https://github.com/github/gitignore) for additional examples across coding languages.

An example config.ini looks like the following, replacing the appropriate placeholders with their respective values:
...
def connect_to_db():
  ...
  Returns:
      (psycopg2.extensions.connection):Postgres database connection.
  '''
  try:
    ...
    conn = psycopg2.connect(
	    host=config['CVTDB']['host'],
	    database=config['CVTDB']['database'],
  	  user=config['CVTDB']['user'],
  	  password=config['CVTDB']['password']
    )
    return conn
...
def query_db(query, inputs, schema):
  '''
  Returns a Pandas DataFrame with the query results.
  Handles errors with Exceptions.

  Parameters:
      query (str):A SQL query string to query the database with.
      inputs (list):A list of inputs when using "WHERE x IN list" clauses.
      schema (str):The schema name to use for the Postgres connection.
  Returns:
      (pd.DataFrame):The queried results in a Pandas DataFrame object.
  '''
  if not query:
...
WHERE e.pmid in (%s)''' % placeholders # Filter to PMID of interest, substituting the list length for inputs by placeholders
