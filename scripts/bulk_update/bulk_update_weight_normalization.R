...
bulk_update_weight_normalization <- function(){
  ...
  query <- paste0("SELECT * FROM cvt.subjects WHERE weight IS NOT NULL")
  
  # Pull data to check
  df_raw <- db_query_cvt(query) %>%
    # Preserve previous conversion value
    dplyr::mutate(weight_old = weight_kg)
  ...
  # Normalized data
  df_normalized <- normalize_weight(raw=df_raw_zip)
  
  # Filter to those where "weight_old" does not equal the new "weight_kg"
  compare <- df_normalized %>% 
    # Round to 3 decimal places to help with comparison
    dplyr::mutate(dplyr::across(c(weight_old, weight_kg), ~round(as.numeric(.), 3))) %>%
    dplyr::filter(xor(is.na(weight_old), is.na(weight_kg))|weight_old != weight_kg)
  
  # Units with conversions before, but not now
  message("Missing old logic unit conversions: ")
  compare %>% 
    dplyr::filter(!is.na(weight_old), is.na(weight_kg)) %>%
  ...
  df_out = df_update %>%
    ...
    tidyr::replace_na(list(weight_old = -99999, weight_kg = -99999)) %>%
    dplyr::filter(weight_old != weight_kg) %>%
    dplyr::filter(weight_kg != -99999) %>%
    dplyr::select(id, weight_kg) %>%
    dplyr::distinct() %>%
    dplyr::mutate(qc_notes = "normalized weight updated")
  ...
  if(nrow(df_out)){
    ...
    message("No weight normalization updates to push...")
  }
