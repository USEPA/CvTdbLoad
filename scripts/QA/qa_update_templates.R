#' @title qa_update_templates
#' @description Workflow to update old templates to current template format (add missing columns).
#' @param in_template_dir Input file directory of templates to update.
#' @return None. XLSX update of template is written.
#' @seealso 
#'  \code{\link[readxl]{excel_sheets}}, \code{\link[readxl]{read_excel}}
#'  \code{\link[dplyr]{mutate}}
#'  \code{\link[writexl]{write_xlsx}}
#' @rdname qa_update_templates
#' @export 
#' @importFrom readxl excel_sheets read_excel
#' @importFrom dplyr mutate
#' @importFrom writexl write_xlsx
qa_update_templates <- function(in_template_dir){
  ...
  addCol <- function(df, newCol, value, neighbor, after=TRUE){
    ...
    message("...adding '", newCol, "' to the ", ifelse(after, "right", "left"), 
            " of '", neighbor, "'")
    if(after){
      return(dplyr::mutate(df, !!newCol := value, .after=!!neighbor))
    } else {
      return(dplyr::mutate(df, !!newCol := value, .before=!!neighbor))
    }
  }
  ...
  f_list = list.files(in_template_dir,
                      pattern=".xlsx",
                      full.names = TRUE,
                      recursive = TRUE)
  ...
  for(f_path in f_list){
    ...
    if(!is.null(f)){
      ...
      if(any(!names(f) %in% names(template))){
        if(!"CompTox" %in% names(f)){# Allow Showa template through
