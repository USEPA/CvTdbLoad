...
def create_template(df, template_name, in_template):
    for sheet_name in in_template.keys():
        ...
        # Find missing cols
        s_missing = list(set(in_template[sheet_name].columns) - 
                         set(s_df.columns))
        ...
        if not os.path.isfile(template_name):
            ...
            with pd.ExcelWriter(template_name, engine="openpyxl") as writer:
                ...
                s_df[in_template[sheet_name].columns.tolist()].to_excel(writer, sheet_name=sheet_name, index=False)
        else:
    ...
    # studies_df = pd.DataFrame(extract_studies_sheet(df, in_template['Studies']))
    # subjects_df = pd.DataFrame(extract_subjects_sheet(df, in_template['Subjects']))
    # series_df = pd.DataFrame(extract_series_sheet(df, in_template['Series']))
    # conc_time_values_df = pd.DataFrame(extract_conc_time_values_sheet(df, in_template['Conc_Time_Values']))

    # # Create excel file
    # with pd.ExcelWriter(template_name, engine="xlsxwriter", mode='a') as writer:
    #     documents_df[in_template['Documents'].columns.tolist()].to_excel(writer, sheet_name="Documents", index=False)
    ...
    for file_path in file_paths:
        try:
            ...
            for i, doc_df in enumerate(doc_dfs):
                ...
                create_template(doc_df, 
                                os.path.join(dest_path, template_name),
                                cvt_template)
        except:
