...
initialize_CvTdb <- function(){
  push_tbl_to_db(dat=data.frame(id=as.numeric(),
                                pmid=as.character(),
...
#' @description Function to write empty tables for CvTdb SQLite file from template docs.
initialize_CvTdb_from_RDat <- function(file=NULL){
  load(file)
  for(t in names(cvt_dat)){
    push_tbl_to_db(dat=cvt_dat[[t]],
                   tblName=t,
                   overwrite=TRUE,
                   fieldTypes = NULL)  
  }
...
#'@import DBI
#'@return None
push_to_CvT <- function(df=NULL, tblName=NULL){
...
get_tbl_id <- function(tblName=NULL, idFilter=NULL){
  if(is.null(tblName)) stop("Must provide database table name to write to")
