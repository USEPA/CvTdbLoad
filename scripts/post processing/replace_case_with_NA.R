...
replace_conc_na <- function(){
  case_dat <- db_query_cvt("SELECT id, conc_original, conc, no_conc_val_type FROM cvt.conc_time_values where no_conc_val_type is NOT NULL and conc_original = 0")# and conc_original is NOT NULL")
