bulk_check_normalization <- function(f="debug_file", log_path="output/debug_log.xlsx"){
  ...
  for(normalization_name in names(query_list)){
    ...
    columns = switch(normalization_name,
                     "normalize_weight" = c("weight", "weight_units"),
                     "normalize_height" = c("height", "height_units"),
                     "normalize_time" = c("time_original", "time_units_original"),
                     "normalize_dose" = c("dose_level", "dose_level_units"),
                     "normalize_conc" = c("conc_original", "conc_units_original", "conc_lower_bound_original", "conc_upper_bound_original")
                     )
