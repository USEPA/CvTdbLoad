...
```{python}
# Chemical list of interest using DTXSID
dtxsid_list = ['DTXSID0021383', 'DTXSID2020139', 'DTXSID0020868', 'DTXSID4020533', 'DTXSID0020442']
# placeholders for a variable number of inputs for the query
placeholders = ', '.join(['%s'] * len(dtxsid_list))

# Select columns of interest
query = """SELECT distinct a.dsstox_substance_id, a.dsstox_casrn, b.radiolabeled, c.test_substance_name_original, b.analyte_name_original, d.administration_route_original
FROM chemicals a 
...
WHERE a.dsstox_substance_id in ({inputs})""".format(inputs=placeholders)
df = query_db(query, dtxsid_list, 'cvt')
df.head()
```

> **Note:** It is important to point out the distinction between the test_substance from the studies table, and the analyte from the series table. The test substance represents the chemical being administered to the subject, whereas the analyte represents the chemical being measured in the subject as a result of the test substance. These substances may be the same, however concentration data may be available for multiple analytes as the result of a single test substance. It may be important to alter your query to search for one or the other depending on your intended uses.
