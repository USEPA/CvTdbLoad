...
if(file.exists("../scripts/release/summarize_cvt_data.R")){
  source("../scripts/release/summarize_cvt_data.R", .GlobalEnv)
  cvt_summary = summarize_cvt_data()
} else {
  stop("Cannot find summarize_cvt_data script...", getwd())
}

v_compare = file.path("../output/release/version_comparison/v2.0.0.RData")
...
|     CvTdb_QC_SOP             |     Guide for the specific process designed for going through previously uploaded templates and updating them for compliance with CvTdb 2024 Release               | .pdf      |
...
### Querying CvTdb
**Add reference and link to the vignette**
...
norm_tbl = list(
  data.frame(
    ...
    category = "weight",
    `normalized units` = "kg",
    fields = "weight, weight_kg, weight_units",
  ...
  data.frame(
    ...
    category = "height",
    `normalized units` = "cm",
    fields = "height, height_cm, height_units",
  ...
  data.frame(
    ...
    fields = "age, age_category, age_units",
  ...
  data.frame(
    ...
    category = "conc",
    `normalized units` = "ug/mL",
    fields = "conc, conc_sd, conc_lower_bound, conc_upper_bound (with *_original fields), conc_units_original (series)",
    ...
    table = "studies",
    category = "dose",
    `normalized units` = "mg/kg",
    fields = "dose_level_original, dose_level_normalized, dose_level_units_original",
  ...
  data.frame(
    ...
    category = "time",
    `normalized units` = "hr",
    fields = "time_original, time_hr, time_units_original (series)",
...
cvt_summary_out = cvt_summary %>%
  ...
  tidyr::unite(col = "summary_out", `Summary Name`, value, 
               sep = ": ", remove = FALSE) %>%
...
Quick notes on past versions:
-   v1.0.0
    -   The original database released with the Sayre, Wambaugh, and Grulke (2020) manuscript.
-   v1.1.0
    -   Release with minor changes to add/update conc_time_values normalizations, fix a couple study normalized administration routes, and add a new chemicals table to split off chemical information from the studies and series tables.

The following tabs of tables summarize count differences between CvTdb versions.
...
vc_stat_list = sort(unique(cvtdb_release_comparison$stat))

# Create list of plots
vc_plots = lapply(vc_stat_list, function(stat){
  if(stat %in% c(
    "Total Administration Routes by Study Count",
    "Total Species by Study Count"
  )) {
...
-   TK Parameter documents (16)
    -   Curation of 43 documents with TK parameters reported, 16 with CvTdb relevant studies, series, and conc_time_values. Note, these were included in the release although they did not complete the standard QC review due to time and resource constraints.
...
-   **NEED TO ADD Reference to Lanacus, first paper to use/reference CvTdb**
...
Special thanks to team members who contributed to CvTdb as a data curator:
Michael F. Hughes, Lucas Albrecht, Grace Cary, Brenda Edwards, Nancy M. Hanley, Anna Jarnagin, Tirumala D. Kodavanti, Evgenia Korol-Bexell, Anna Kreutz, Mayla Ngo, Caitlyn Patullo, Hiroshi Yamazaki, Evelyn G. Rowan, L. McKenna Huse, Veronica A. Correa, Branislav Kesic, Robert "Will" Casey, Jennat Aboabdo, Kaitlyn Wolf, Jasmine Hope.
