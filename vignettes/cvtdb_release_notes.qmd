...
cvt_summary_out = cvt_summary %>%
  ...
  dplyr::mutate(summary_out = dplyr::case_when(
    # Sub-bullet
    `Summary Name` %in% c(
      "Total Dosed Chemicals",
      "Total Dosed Chemicals Mapped to DTXSID",
      "Species Breakdown",
      "Total Analyzed Chemicals",
      "Total Analyzed Chemicals Mapped to DTXSID",
      "Total Concentration Time Values with Normalized Units"
    ) ~ paste0("\t-\t", summary_out),
    # Parent bullet
...
```{r}
#| label: "cvt-summary-visualizations"
#| echo: FALSE
#| results: 'asis'
...
chemical_route_summary = cvt_summary_query %>%
  ...
  dplyr::distinct() %>%
  dplyr::filter(!is.na(dosed_chem_dtxsid), !is.na(administration_route_normalized)) %>%
...
chemical_species_summary = cvt_summary_query %>%
  ...
  dplyr::distinct() %>%
  dplyr::filter(!is.na(dosed_chem_dtxsid), !is.na(species)) %>%
...
-   New/updated dermal study metadata model.
-   New/updated inhalation study metadata model.
...
-   \*\*Add reference to Gil's paper and others?
